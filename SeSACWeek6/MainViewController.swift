...
/*
 awakeFromNib - 셀 UI 초기화, 재사용 매커니즘에 의해 일정횟수 이상 호출되지 않음
 cellForItemAt
 - 재사용될 때마다, 사용자에게 보일 때 마다 항상 실행됨
 - 화면과 데이터는 별개, 모든 indexPath.item에 대한 조건이 없다면 재사용 시 오류가 발생할 수 있음. ex) if indexPath.item < 2 {
...
- 복합적인 구조라면, 테이블셀도 재사용되어야하고, 컬렉션셀도 재사용 되어야 함.
 */
...
extension MainViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    //테이블뷰 객체가 하나일 경우에는 내부 매개변수를 활용하지 않아도 문제가 생기지 않는다. (여기서 만약 mainTableView.로 특정해도 문제 생기지 않음)
    //하지만, 여러개일 경우에는 문제임.
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.contentCollectionView.reloadData() //Index Out of Range 오류 해결. tableview가 먼저 호출되니까 여기서 갱신을 해야함. 만약 컬렉션 뷰 안에 테이블뷰가 있다면 거기서 reload를 해줘야될듯
        //단 데이터 개수랑 인덱스 번호가 같다면 굳이 reloadData할 필요 없음
...
extension MainViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    //bannerCollectionView OR 테이블뷰 안에 들어있는 컬렉션뷰 -> 둘 다 들어올 수 있음.
    //내부 매개변수가 아닌 명확한 아웃렛을 사용할 경우, 셀이 재사용되면 특정 collectionView셀을 재사용하게 될 수 있음. (예를 들어 여기에서는 bannerCollectionView로 특정하면 빌드시 재사용을 이상하게 함)
    //(여기서 bannerCollectionView를 재사용하려는 목적이 아니라 내부매개변수를 재사용하려는 목적이라서)
